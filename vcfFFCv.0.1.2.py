...
###
### VCF Filtering and Fasta Creation Version 0.1.2
###
...
Citation:       Biancamaria Bonucci, Toni de-Dios, Rémi Barbieri, Jess Emma Thompson, Sofia Panella, Francesca Radina, Sandra Savilli, Helja Kabral, Anu Solnik, Mary Anne Tafuri, John Robb, Christiana Lyn Scheib
                ...
                Available at SSRN: https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5010900

Usage:          python vcfFFCv0.1.2.py -i <Sorted Multi Sample VCF> -o <Output file> -r <Heterozyogus Ratio> 
                                     -p <Min SNP distance in BP> -q <Min Calling Quality> -d <Min Depth> 
                                     -m <Min % of covered SNPs> --exlude-transitions 
...
parser.add_argument( "-a", "--transitions",             dest = "exclude_transitions", action="store_false"        )
parser.add_argument( "-t", "--exclude-transitions",     dest = "exclude_transitions", action="store_true", help="Remove genetic transitions" )

parser.set_defaults(exclude_transitions=False)
...
### Arguments
exclude_transitions=args.exclude_transitions
try:    ### Open VCF
...
for line in vcf: ### Reading VCF
    ...
    if li.startswith("#"):  
        ...
        if "#CHROM" in line:
            ...
            to_append=["Reference"]
            for element in (header[9:(len(header))]):
                to_append.append(element)

            sampleRef.append(to_append)
...
sampleRef=[x for y in sampleRef for x in y]
sample=[n for m in sample for n in m]
...
for i in range(0,len(heterozyosity)):
    ...
    result_stats=[str(sample[i]),"\t",str((heterozyosity[i])*100),"\t",str(100-((str(fasta[(i+1)])).count("N")/len(str(fasta[(i+1)]))*100)),"\t",str(len(str(fasta[(i+1)])))]
...
for j in range(0,len(sampleRef)):
    result_fasta=[">",str(sampleRef[j]),"\n",str(fasta[j])]
